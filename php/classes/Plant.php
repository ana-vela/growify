...
class Plant implements \JsonSerializable{
	...
	public static function getPlantByPlantName(\PDO $pdo, string $plantName) {
		...
		// create query template
		$query = "SELECT plantId, plantLatinName, plantName, plantVariety, plantDescription, plantType, plantSpread, plantHeight, plantDaysToHarvest, plantMinTemp, plantMaxTemp, plantSoilMoisture FROM plant WHERE plantName LIKE :plantName";
		$statement = $pdo->prepare($query);
